...
public:
  ...
  cl_command_queue CreateCommandQueue(cl_context context,
    ...
    commandQueue =
        clCreateCommandQueueWithProperties(context, devices[0], 0, NULL);
  ...
  bool CreateMemObjects_init(cl_context context, cl_mem memObjects[9],
                             double *h, float *m_Mean, float *m_Var,
                             float *m_Age, float *m_Mean1, float *m_Var1,
                             float *m_Age1, float *image, float *mmask) {
    memObjects[0] =
  ...
  bool CreateMemObjects(cl_context context, cl_mem memObjects[9], double *h,
                        float *image, float *mmask) {
    memObjects[0] =
        clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                       sizeof(double) * 16 * 9, h, NULL);
    memObjects[7] =
  ...
  void motionCompensate_init(double *h, const Mat &pOutputImg) {
    ...
    CreateMemObjects_init(context, memObjects, h, m_Mean[0], m_Var[0], m_Age[0],
                          m_Mean[1], m_Var[1], m_Age[1], image, mmask);
    ...
    errNum =
        clEnqueueReadBuffer(commandQueue, memObjects[8], CL_TRUE, 0,
                            1920 * 1080 * sizeof(float), mmask, 0, NULL, NULL);
