...
KLTWrapper::group_points(vector<Point2f> good0, vector<Point2f> good1) {
...
void KLTWrapper::InitFeatures() {
  ...
  for (int i = 0; i < count; ++i) {
    prev_pts.push_back(cv::Point2f(
        float(i) / (int(nj / GRID_SIZE_H) - 1) * GRID_SIZE_W + GRID_SIZE_W / 2,
        i % (int(nj / GRID_SIZE_H) - 1) * GRID_SIZE_H + GRID_SIZE_H / 2));
  }
}
...
void KLTWrapper::RunTrack(const UMat &imgGray, const UMat &prevGray) {
  ...
  if (count > 0) {
    ...
    cv::calcOpticalFlowPyrLK(
        prevGray, imgGray, prev_pts, umatNextPts, umatStatus, umatErr,
        Size(8, 8), 1,
        cv::TermCriteria((TermCriteria::COUNT) | (TermCriteria::EPS), 20,
                         (0.03)),
        0);
    ...
    for (int i = 0; i < prev_pts.size(); i++) {
      if (!status[i] || next_pts[i].x < 0 || next_pts[i].y < 0 ||
          next_pts[i].x > width || next_pts[i].y > height) {
        continue;
      }
...
void KLTWrapper::MakeHomoGraphy(std::vector<cv::Point2f> &good0,
                                std::vector<cv::Point2f> &good1) {
