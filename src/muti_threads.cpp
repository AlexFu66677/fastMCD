...
//   VideoCapture cap1("/home/fjl/code/moving/fusebbox_SGM/python/data/car2.mp4");
//   UMat frame;
//   UMat U_imgGray;
//   cap1 >> frame;
//   cv::cvtColor(frame, U_imgGray, CV_RGB2GRAY);
//   Mat mat_imgGray;
//   vector<Tracker> bgs_tracked_list;
//   U_imgGray.copyTo(mat_imgGray);
//   BGModel.init(mat_imgGray);
//   double *h = new double[16 * 9];
//   UMat imggrayPrev = U_imgGray.clone();
//   while (true) {
//     {
//       std::lock_guard<std::mutex> lock(mtx);
//       if (frameQueue.empty()) {
...
//     cap1 >> cur_frame;
//     UMat imggray;
//     cv::cvtColor(cur_frame, imggray, CV_RGB2GRAY);
//     Mat Temp;
//     imggray.copyTo(Temp);
...
//     UMat thresh;
//     thresh = UMat::zeros(cur_frame.rows, cur_frame.cols, CV_8UC1);
...
//       for (int i = 0; i < 16 * 9; i++) {
//         h[i] = frameQueue.front()[i];
//         // data["data"].append(frameQueue.front()[i]);
//       }
//       frameQueue.pop();
//     }
//     Mat cur_frame;
